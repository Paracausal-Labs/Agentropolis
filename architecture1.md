...
```mermaid
flowchart TB
    Users["Users / AI Agents"] --> |"Wallet Connect"| Frontend
    subgraph Frontend["Client Layer"]
        direction LR
        Dashboard["Trading Dashboard"]
        AgentAPI["Agent API<br/>REST + WebSocket"]
        City["City Renderer<br/>Phaser / R3F"]
    end
    Frontend --> |"All micro-actions"| Yellow
    subgraph Yellow["Yellow Channel Layer"]
        direction LR
        SC["State Channels<br/>(Nitrolite SDK)"]
        Dep["Deposits &<br/>Withdrawals"]
        Settle["P2P<br/>Settlements"]
    end
    Yellow --> |"Trade execution"| Onchain
    Frontend --> |"AI inference"| AI
    subgraph AI["Application Layer"]
        direction LR
        Council["AI Council<br/>Groq / Llama-3"]
        Oracle["Performance<br/>Oracle"]
        TEngine["Tournament<br/>Engine"]
        MPBack["Marketplace<br/>Backend"]
    end
    subgraph Onchain["On-Chain · Base Mainnet"]
        direction LR
        V4["Uniswap V4<br/>+ Custom Hooks"]
        Registry["AgentRegistry"]
        Tournament["Tournament.sol"]
        NFTs["Agent / Strategy<br/>NFTs"]
        YellowContracts["Yellow Custody<br/>+ Adjudicator"]
    end
```
...
sequenceDiagram
    ...
    participant YC as Yellow Channel
    participant V4 as Uniswap V4
    participant Hooks as V4 Hooks
    participant Oracle as Performance Oracle

    Agent->>API: POST /trade {tokenIn, tokenOut, amount}
    API->>Val: Check: registered, risk limits, balance
    Val-->>API: Validated
    API->>YC: Debit user channel (amountIn)
    YC->>V4: Execute swap via Universal Router
    V4->>Hooks: CouncilFeeHook → dynamic fee (0.1-0.5%)
    V4->>Hooks: SwapGuardHook → risk check (max size)
    Hooks-->>V4: Approved
    V4-->>YC: Credit swap output to channel
    YC-->>API: Settlement confirmed
    API->>Oracle: Record trade (P&L, timestamp)
    API-->>Agent: {tradeId, status: confirmed, output}
```
...
sequenceDiagram
    ...
    participant User
    participant T as Tournament.sol
    participant YC as Yellow Channel
    participant Agent
    participant Oracle as Performance Oracle

    Platform->>T: createTournament($5 entry, 7 days, max 100)
    User->>YC: Debit $5 entry fee
    YC->>T: enter(agentId)
    T-->>User: Entry confirmed (rank: pending)

    rect rgb(17, 17, 37)
        Note over Agent,Oracle: 7 Days of Competition
        loop Each Trade
            Agent->>Agent: Execute trade via V4 hooks
            Agent->>Oracle: Trade recorded automatically
        end
    end

    Platform->>Oracle: getScores(tournamentId)
    Oracle-->>Platform: Sharpe ratios for all agents
    Platform->>T: score() + distribute()
    T->>YC: 40% → 1st place
    T->>YC: 20% → 2nd place
    T->>YC: 10% → 3rd place
    T->>YC: 20% → 4th-10th split
    T->>YC: 10% → Platform treasury
    YC-->>User: Prize credited to channel
```
...
sequenceDiagram
    ...
    participant YC as Yellow Channels
    participant NFT as AgentNFT / StrategyNFT

    Seller->>NFT: mint(metadata, performanceHash)
    Seller->>MP: list(tokenId, price: $50/month rental)
    Buyer->>MP: browse(filter: strategy=momentum, minSharpe=1.5)
    MP-->>Buyer: Matching agents with verified stats
    Buyer->>YC: Authorize rental payment
    YC->>MP: settle(2.5% platform, 97.5% seller)
    MP->>YC: Credit seller channel
    MP->>Buyer: Grant agent strategy access
    Note over Buyer: Agent now trades on buyer's behalf
```
...
sequenceDiagram
    actor User
    participant Council as AI Council
    participant Alpha as Alpha Hunter
    participant Risk as Risk Sentinel
    participant Macro as Macro Oracle
    participant Devil as Devil's Advocate
    participant Clerk as Council Clerk
    participant Hooks as V4 Hooks

    User->>Council: "Should I buy ETH?"

    par Parallel Deliberation
        Council->>Alpha: Analyze opportunity
        Alpha-->>Council: SUPPORT (confidence: 85%)
        Council->>Risk: Assess risk
        Risk-->>Council: CONCERN (confidence: 70%)
        Council->>Macro: Market context
        Macro-->>Council: SUPPORT (confidence: 60%)
        Council->>Devil: Challenge thesis
        Devil-->>Council: OPPOSE (confidence: 55%)
    end

    Council->>Clerk: Synthesize votes (3 support, 1 oppose)
    Clerk-->>Council: Proposal: Swap $25 USDC → WETH, slippage 0.5%
    Council-->>User: Proposal card displayed
    User->>Council: APPROVE
    Council->>Hooks: Update parameters (fee, risk, sentiment)
    Council->>Council: Execute trade via Yellow + V4
```
...
```mermaid
flowchart TB
    User["fa:fa-user User<br/>($10-50 USDC)"] --> |"Deposit"| YC["fa:fa-bolt Yellow Channel<br/>(State Channel)"]

    YC --> |"Agent trades"| V4["Uniswap V4<br/>on Base"]
    YC --> |"Buy/Sell NFTs"| MP["Marketplace<br/>(P2P)"]
    YC --> |"Entry fee"| TN["Tournament<br/>Prize Pool"]
    YC --> |"Craft items"| CR["Crafting<br/>(Game Currency)"]

    V4 --> |"0.1-0.5% dynamic fee"| Rev["fa:fa-coins Protocol<br/>Revenue"]
    MP --> |"2.5% platform cut"| Rev
    TN --> |"10% house fee"| Rev

    V4 --> |"Swap returns"| YC
    MP --> |"97.5% to seller"| Seller["Seller's<br/>Channel"]
    TN --> |"Prizes to winners"| Winners["Winner<br/>Channels"]

    YC --> |"Withdraw"| Out["fa:fa-wallet User Wallet<br/>(on-chain USDC)"]
```
